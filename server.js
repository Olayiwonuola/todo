...
let sanitzeHTML = require("sanitize-html");
...
async function go() {
  let client = new MongoClient(
    "mongodb+srv://jnrwaleade:0Mse7Q1rYxhongfe@cluster0.tpbwlvc.mongodb.net/todoApp?retryWrites=true&w=majority"
  );
...
function passwordProtected(req, res, next) {
  ...
  if (req.headers.authorization == "Basic ZnVsbDpzdGFjaw==") {
    next();
  } else {
...
app.post("/create-item", async function (req, res) {
  let safeText = sanitzeHTML(req.body.text, {
    allowedTags: [],
    allowedAttributes: {},
  });
